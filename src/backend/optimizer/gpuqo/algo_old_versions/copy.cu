...
// while (!queue.empty()) {
//     pair top_triplet = queue.top();
//     int low_key_iter = std::get<0>(top_triplet);
//     float end_iter = std::get<1>(top_triplet);
//     queue.pop();
//     ...
// }
#include <queue>
...
// using pair_t = std::tuple<BitmapsetN, int, float>;
...
// for second priority queue (of unions)
template<typename BitmapsetN>
struct UnionNode{
    BitmapsetN union_id;
	BitmapsetN node_id;
	int size; //TODO: This needs to be created with ds.getSize(node_id) of DisjointSet or else it will be wrong
	float edge_weight; // minimum weight apo ta edges tou node_id
};

// min priority queue mou kanei to > kai max priority queue mou ftiaxnei to <
template<typename BitmapsetN>
struct LeafPriorityComparator{
...
template<typename BitmapsetN>
struct UnionPriorityComparator{
    bool operator()(const GraphEdge<BitmapsetN>& lhs, const GraphEdge<BitmapsetN>& rhs) {
        return  lhs.size > rhs.size || (lhs.size == rhs.size && lhs.edge_weight > rhs.edge_weight);
    }
};

//TODO: This should be created during BFS?
void createLeafQueue(GpuqoPlannerInfo<BitmapsetN>* info){
    std::priority_queue<LeafNode, std::vector<LeafNode>, LeafPriorityComparator> leaf_queue;

    for(i=0; i<info->n_rels; i++){
        if (info->edge_table[i.size() == 1]) 
        {
            LeafNode leaf;
            leaf.id = info->base_rels[i].id;
            leaf.edge_weight = 
        }
    }
    // iterate over all edges and get the rank by saying bms.size() == 1 -> leaf

    return leaf_queue; // should it return a leaf priority queueu? probably
}


void main(){
    // using pair = pair_t<int>;

    // to priority queue vazei max panw
    // alla o comparator exei sto kefali tou oti einai ress
    // diladi to True einai oti to prwto einai less apo to deutero
    // ARA opws to exw edw einai 
    auto comparator = [](const GraphEdge<BitmapsetN>& lhs, const GraphEdge<BitmapsetN>& rhs) {
        return std::get<1>(it1) > std::get<1>(it2) ||
