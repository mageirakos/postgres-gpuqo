									...
									int n_iters)
{
	...
	// printf("\n\tSTART of IDP\n\n");
	// list<remapper_transf_el_t<BitmapsetN> > remap_list_2;
	// for (int i=0; i<info->n_rels; i++){
	// 	remapper_transf_el_t<BitmapsetN> list_el;
	// 	list_el.from_relid = info->base_rels[i].id;
	// 	list_el.to_idx = i;
	...
	// 	remap_list_2.push_back(list_el);
	// }
	// QueryTree<BitmapsetN> *final_out_qt = gpuqo_run_idp2_rec<BitmapsetN,BitmapsetN>(
	// 					gpuqo_algo, out_qt, info, remap_list_2, 
	// 					n_iters > 0 ? n_iters : gpuqo_idp_n_iters);

	// freeQueryTree(out_qt);
    // level_of_rec = 0;

	// printf("\tOUT OF RECURSION\n");
	// std::cout << "UNION Join Tree Cost: " << out_qt->cost.total << std::endl;
	// std::cout << "FINAL UNION and IDP Join Tree Cost: " << final_out_qt->cost.total << std::endl;
