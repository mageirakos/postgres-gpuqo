...
class SnowflakeSchema:
    ...
    def __make_create_table(self, id, leaf=False):
        ...
        if columns:
            columns = ",\n    " + columns
        blah =  TABLE_PATTERN.format(id_to_str(id), columns)
        # print(blah)
        return blah
    ...
    # STAR Defaults (10k-1M for dimension) (10M for fact)
...
if __name__ == "__main__":
    ...
    schema = SnowflakeSchema((1, 250, 0),
                             (0, 250, 0))
    ...
    for n in tqdm(range(100,251,50)):
        for i in range(104):
            with open(f"queries/{n:04d}{labels[i]}.sql", 'w') as f:
                f.write(schema.make_query(n))
                f.write("\n")
